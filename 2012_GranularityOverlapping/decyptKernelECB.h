...
size_t inputSize, uint32_t* dev_sm_td1,
uint32_t* dev_sm_td2, uint32_t* dev_sm_td3, uint32_t* dev_sm_td4 )
{
    ...
    if(global_tid < inputSize/16) {
        ...
        if (tid<256){
            ...
            // const_sm_sbox_inv to share memory variables sm_td1, sm_td2,
            //sm_td3, sm_td4 and sm_isbox;
            sm_td1[global_tid]      = dev_sm_td1[global_tid];
            sm_td2[global_tid]      = dev_sm_td2[global_tid];
            sm_td3[global_tid]      = dev_sm_td3[global_tid];
            sm_td4[global_tid]      = dev_sm_td4[global_tid];
        ...
        #pragma unroll
        for (int i = 1; i < 10; i++) {
            s1 = (sm_td1[w1 >> 24] ^ sm_td2[(w2 >> 16) & 0xFF] ^
            sm_td3[(w3 >> 8) & 0xFF] ^ sm_td4[w4 & 0xFF]) ^
            const_m_ke[i*4];
            w1 = s1;

            s2 = (sm_td1[w1 >> 24] ^ sm_td2[(w2 >> 16) & 0xFF] ^
            sm_td3[(w3 >> 8) & 0xFF] ^ sm_td4[w4 & 0xFF]) ^
            const_m_ke[i*4+1];
            w2 = s2;
            
            s3 = (sm_td1[w1 >> 24] ^ sm_td2[(w2 >> 16) & 0xFF] ^
            sm_td3[(w3 >> 8) & 0xFF] ^ sm_td4[w4 & 0xFF]) ^
            const_m_ke[i*4+2];
            w3 = s3;
            
            s4 = (sm_td1[w1 >> 24] ^ sm_td2[(w2 >> 16) & 0xFF] ^
            sm_td3[(w3 >> 8) & 0xFF] ^ sm_td4[w4 & 0xFF]) ^
            const_m_ke[i*4+3];
            w4 = s4;
        ...
        w1 = (uint32_t)(sm_isbox[ s1 >> 24 ]) << 24; //SubBytes and ShiftRows
        w1 |= (uint32_t)(sm_isbox[(s2 >> 16)& 0xFF]) << 16;
        w1 |= (uint32_t)(sm_isbox[(s3 >> 8)& 0xFF]) << 8 ;
        w1 |= (uint32_t)(sm_isbox[s4 & 0xFF]);
        w1 ^= const_m_ke[ROUNDS*4]; //AddRoundKey
        w1 =( (w1>>24)&0xFF)|(( w1>>8)&0xFF00)|( (w1<<8)&0xFF0000)
        |( (w1<<24)&0xFF000000;
        dev_output[4*global_tid] = w1 ; //store the cipher text
        
        w2 = (uint32_t)(sm_isbox[ s1 >> 24 ]) << 24; //SubBytes and ShiftRows
        w2 |= (uint32_t)(sm_isbox[(s2 >> 16)& 0xFF]) << 16;
        w2 |= (uint32_t)(sm_isbox[(s3 >> 8)& 0xFF]) << 8 ;
        w2 |= (uint32_t)(sm_isbox[s4 & 0xFF]);
        w2 ^= const_m_ke[ROUNDS*4]; //AddRoundKey
        w2 =( (w2>>24)&0xFF)|(( w2>>8)&0xFF00)|( (w2<<8)&0xFF0000)
        |( (w2<<24)&0xFF000000;
        dev_output[4*global_tid+1] = w2 ; //store the cipher text

        w3 = (uint32_t)(sm_isbox[ s1 >> 24 ]) << 24; //SubBytes and ShiftRows
        w3 |= (uint32_t)(sm_isbox[(s2 >> 16)& 0xFF]) << 16;
        w3 |= (uint32_t)(sm_isbox[(s3 >> 8)& 0xFF]) << 8 ;
        w3 |= (uint32_t)(sm_isbox[s4 & 0xFF]);

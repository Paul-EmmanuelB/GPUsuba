...
size_t inputSize, uint32_t* dev_sm_te1, uint32_t* dev_sm_te2, uint32_t* dev_sm_te3, uint32_t* dev_sm_te4,  uint8_t* dev_sm_sbox)
{
    ...
    if(global_warpID < inputSize/512) {
        ...
        dev_output[128*global_warpID+laneID+0] = s1 ; //store the cipher text
        ...
        dev_output[128*global_warpID+laneID+32] = s2 ; //store the cipher text
        ...
        dev_output[128*global_warpID+laneID+64] = s3 ; //store the cipher text
        ...
        dev_output[128*global_warpID+laneID+96] = s4 ; //store the cipher text
    }
}

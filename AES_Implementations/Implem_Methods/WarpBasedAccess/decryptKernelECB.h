...
size_t inputSize, uint32_t* dev_sm_td1, uint32_t* dev_sm_td2, uint32_t* dev_sm_td3, uint32_t* dev_sm_td4, uint8_t* dev_sm_isbox_inv)
{
    ...
    if(warpID < warps) {
        ...
        s4 = dev_input[128*warpID+laneID+warpSize*3];
        ...
        w1 ^= const_expkey[0];
        dev_output[128*warpID+laneID+warpSize*0] = w1 ; //store the cipher text
        ...
        w2 ^= const_expkey[1];
        dev_output[128*warpID+laneID+warpSize*1] = w2 ; //store the cipher text
        ...
        w3 ^= const_expkey[2];
        dev_output[128*warpID+laneID+warpSize*2] = w3 ; //store the cipher text
        ...
        w4 ^= const_expkey[3];
        dev_output[128*warpID+laneID+warpSize*3] = w4 ; //store the cipher text
    }

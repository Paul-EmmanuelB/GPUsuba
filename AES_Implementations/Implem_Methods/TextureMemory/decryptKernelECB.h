...
__global__ static void decrypt_Kernel( uint32_t* dev_input, uint32_t* dev_output, size_t inputSize, uint8_t* dev_sm_isbox,
cudaTextureObject_t tex0, cudaTextureObject_t tex1, cudaTextureObject_t tex2, cudaTextureObject_t tex3)
{
    ...
    if ( !elemPerThread && tid<256) {
        ...
        // dev_sm_isbox to share memory variables sm_td1, sm_td2,
        //sm_td3, sm_td4 and sm_isbox;
        sm_td1[tid]      = tex1Dfetch<uint32>(tex0, tid);
        sm_td2[tid]      = tex1Dfetch<uint32>(tex1, tid);
        sm_td3[tid]      = tex1Dfetch<uint32>(tex2, tid);
        sm_td4[tid]      = tex1Dfetch<uint32>(tex3, tid);
        sm_isbox[tid]    = dev_sm_isbox[tid];
    }
    else {
        for(int i=0; i<elemPerThread; i++) {
            sm_td1[tid*elemPerThread  + i]   = tex1Dfetch<uint32>(tex0, tid*elemPerThread + i);
            sm_td2[tid*elemPerThread  + i]   = tex1Dfetch<uint32>(tex1, tid*elemPerThread + i);
            sm_td3[tid*elemPerThread  + i]   = tex1Dfetch<uint32>(tex2, tid*elemPerThread + i);
            sm_td4[tid*elemPerThread  + i]   = tex1Dfetch<uint32>(tex3, tid*elemPerThread + i);
            sm_isbox[tid*elemPerThread + i]   = dev_sm_isbox[tid*elemPerThread + i];
        }
        int modEPT = 256%blockSize; //256 is not a multiple of blockSize
        if(!modEPT && (tid == blockSize-1)) {
            for(int i=0; i<modEPT; i++) {
                sm_td1[tid*(elemPerThread+1)  + i]   = tex1Dfetch<uint32>(tex0, tid*(elemPerThread+1) + i);
                sm_td2[tid*(elemPerThread+1)  + i]   = tex1Dfetch<uint32>(tex1, tid*(elemPerThread+1) + i);
                sm_td3[tid*(elemPerThread+1)  + i]   = tex1Dfetch<uint32>(tex2, tid*(elemPerThread+1) + i);
                sm_td4[tid*(elemPerThread+1)  + i]   = tex1Dfetch<uint32>(tex3, tid*(elemPerThread+1) + i);
                sm_isbox[tid*(elemPerThread+1) + i]  = dev_sm_isbox[tid*(elemPerThread+1) + i];
            }
        }
    }
    __syncthreads();

...
int main(int argc, char * argv[]) {
    ...
    uint4    * devInput, * devOutput; 
    uint32_t * dev_sm_te1, * dev_sm_te2, * dev_sm_te3, * dev_sm_te4;
    uint8_t  * dev_sm_sbox;
    ...
    int stateBlocks = filesize/16;
    //Warm Up
    if(mode) {
        for(int i=0; i < warm_up_device ; i++) { 
            encrypt_Kernel<<<blockNum,threadNum>>>(devInput, devOutput, stateBlocks, dev_sm_te1,
        dev_sm_te2, dev_sm_te3, dev_sm_te4, dev_sm_sbox);
        }
    }
    else {
        for(int i=0; i < warm_up_device ; i++) {
                decrypt_Kernel<<<blockNum,threadNum>>>(devInput, devOutput, stateBlocks, dev_sm_te1,
            dev_sm_te2, dev_sm_te3, dev_sm_te4, dev_sm_sbox);
        }
    }
